...
export default function HeroButtons({ value, styles }: { value: string, styles?: string }) {
    ...
    return (
        <div id={`${value}`} onMouseEnter={mouseEnter} onMouseLeave={mouseLeave} className={`${activeGradient} flex items-center justify-center w-full border-2 rounded-2xl border-[red] p-3 text-[#fdfaed] hover:border-black ${styles ? styles : ""}`}>
